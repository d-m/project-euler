...
def euler():
    SUM_TRIANGLE = list(reversed(TRIANGLE))
    for row1, row2 in pairwise(SUM_TRIANGLE):
        for index, item in enumerate(row2):
            row2[index] = item + max(row1[index], row1[index + 1])
    print(list(reversed(SUM_TRIANGLE))[0][0])
